...
template<class T_Material>
inline MeshRenderer<T_Material>::MeshRenderer(std::shared_ptr<GameObject> pParent, const std::string& filePath)
...
template<class T_Material>
inline MeshRenderer<T_Material>::~MeshRenderer()
{
...
template<class T_Material>
inline void MeshRenderer<T_Material>::Render(ID3D11DeviceContext* pDeviceContext) const
{
...
template<class T_Material>
inline void MeshRenderer<T_Material>::SetMatrix(const Matrix& matrix) const
{
	if (m_pTransform.expired()) return;

	const Matrix worldMatrix{ m_pTransform.lock()->GetTransformationMatrix()};

	m_pMaterial->SetMatrix(worldMatrix * matrix);
}
