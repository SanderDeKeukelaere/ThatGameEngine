#include "pch.h"
#include "CameraNoComponent.h"

namespace dae
{
	CameraNoComponent::CameraNoComponent(const Vector3& _origin, float _fovAngle) :
		m_Origin{ _origin }
	{
		ChangeFOV(_fovAngle);
	}

	void CameraNoComponent::Initialize(float _fovAngle, Vector3 _origin, float _aspectRatio)
	{
		m_Origin = _origin;
	...
	void CameraNoComponent::CalculateViewMatrix()
	{
	...
	void CameraNoComponent::ChangeFOV(float newFov)
	{
		m_FovAngle = newFov;
